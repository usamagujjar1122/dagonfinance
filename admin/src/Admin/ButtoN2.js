...
const ButtoN2 = ({type,item,setAlert, setMsg, setType,setrefresh}) => {
    ...
    return (
        <>
            <Button sx={{ color: '#868e96', backgroundColor: '#e9ecef', borderRadius: '20px', padding: {xs:"5px 10px",md:'5px 16px'}, '&:hover': { backgroundColor:type==='complete'||type==='approve'? '#28a745': '#dc3545', color: 'white' } }} onClick={() => { clicked(item) }}>
                {loading ?
